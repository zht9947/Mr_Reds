...
!***********************************************************************************************************************************
  ...
  & NND , EXTEND_NND , NEM , EXTEND_NEM , NLOOPS , NI , NJ , ERROR , ERRMSG )
    ...
    INTEGER(NDID_KIND) , INTENT(IN ) :: NND , EXTEND_NND
    INTEGER(EMID_KIND) , INTENT(IN ) :: NEM , EXTEND_NEM
    ...
    ELSE
      ALLOCATE( PATH_UV_CHAR_ARRAY( N_UV_DSETS * N_MAX_PATH_UV_LENGTH ) )
        ...
        ELSE
          ...
          ! READ FROM SOURCE
            CALL MR_GEN_READ_UV( MULTI_DSETS_ID , PATH_UV(1:N_MAX_PATH_UV_LENGTH-1) , ITS ,   &
          ...
          ! WRITE TO THE TARGET
            CALL MR_GEN_WRITE_UV( MULTI_DSETS_ID_EXTEND , PATH_UV(1:N_MAX_PATH_UV_LENGTH-1) , T ,   &
            & EXTEND_NND , EXTEND_NEM , NI*NLOOPS , NJ , EXTEND_EMIDW , EXTEND_NDIDW , EXTEND_NDIDU , EXTEND_NDIDV , EXTEND_NDIDO ,   &
    ...
    ELSE
      ALLOCATE( PATH_SS_CHAR_ARRAY( N_SS_DSETS * N_MAX_PATH_SS_LENGTH ) )
        ...
        ELSE
          ...
          ! READ FROM SOURCE
            CALL MR_GEN_READ_SS( MULTI_DSETS_ID , PATH_SS(1:N_MAX_PATH_SS_LENGTH-1) , ITS ,   &
            & NND , NEM , NI , NJ , EMIDW , NDIDW , NDIDU , NDIDV , NDIDO ,   &
            & SS , SU, SV , SO ,   &
            & ACTIVITY , ERROR , ERRMSG )
            IF( ERROR < 0 ) THEN
          ...
          ! WRITE TO THE TARGET
            CALL MR_GEN_WRITE_SS( MULTI_DSETS_ID_EXTEND , PATH_SS(1:N_MAX_PATH_SS_LENGTH-1) , T ,   &
            & EXTEND_NND , EXTEND_NEM , NI*NLOOPS , NJ , EXTEND_EMIDW , EXTEND_NDIDW , EXTEND_NDIDU , EXTEND_NDIDV , EXTEND_NDIDO ,   &
