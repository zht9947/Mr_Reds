    ...
    ELSE
      ALLOCATE( PATH_UV_CHAR_ARRAY( N_UV_DSETS * N_MAX_PATH_UV_LENGTH ) )
        ...
        ELSE
          ...
          ! WRITE TO THE TARGET
            CALL MR_GEN_WRITE_UV( MULTI_DSETS_ID_RESAMPLE , PATH_UV(1:N_MAX_PATH_UV_LENGTH-1) , T ,   &
            & NND , NEM , NI , NJ , EMIDW , NDIDW , NDIDU , NDIDV , NDIDO ,   &
            & UV , UU , VV , UVO ,   &
            & ACTIVITY , ERROR , ERRMSG )
            IF( ERROR < 0 ) THEN
    ...
    ELSE
      ALLOCATE( PATH_SS_CHAR_ARRAY( N_SS_DSETS * N_MAX_PATH_SS_LENGTH ) )
        ...
        ELSE
          ...
          ! WRITE TO THE TARGET
            CALL MR_GEN_WRITE_SS( MULTI_DSETS_ID_RESAMPLE , PATH_SS(1:N_MAX_PATH_SS_LENGTH-1) , T ,   &
            & NND , NEM , NI , NJ , EMIDW , NDIDW , NDIDU , NDIDV , NDIDO ,   &
            & SS , SU , SV , SO ,   &
            & ACTIVITY , ERROR , ERRMSG )
            IF( ERROR < 0 ) THEN
