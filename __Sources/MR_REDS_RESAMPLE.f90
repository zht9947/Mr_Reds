...
!***********************************************************************************************************************************
  ...
  ! ALLOCATE MEMORIES
   ...
   !END BLOCK
    ...
    CALL MR_INIT_GRID_SYS( FILE_XMDF_RESAMPLE , ERROR , ERRMSG )
    IF( ERROR < 0 ) THEN
      WRITE(*,'(/,2X, A ,"!")') TRIM(ERRMSG)
      STOP
    END IF
  ...
  ! REASSIGN ITS_START
    ...
    ! GET T OF ITS
      ...
      CALL MR_RW_RESAMPLE( FILE_XMDF , FILE_XMDF_RESAMPLE , ITS , T , ERROR , ERRMSG )
      IF( ERROR < 0 ) THEN
        WRITE(*,'(//,2X, A ,"!")') TRIM(ERRMSG)
        STOP
      END IF
      
     WRITE(*,'(8X,"Resample...",F6.2,"%", A , $ )')   &
    ...
    WRITE(*,'(/,"The result has been written into the file: ",/,4X, A )') TRIM(FILE_XMDF_RESAMPLE)
...
!***********************************************************************************************************************************
  ...
  ! GET XMDF_RESAMPLE FILE'S PATH\NAME
    CALL GET_COMMAND_ARGUMENT( 2 , FILE_XMDF_RESAMPLE , STATUS=ERROR )
    IF( ERROR == - 1 ) THEN
      ERRMSG = "XMDF_RESAMPLE File's path too long!"
      RETURN
    ELSE IF( ERROR /= 0 ) THEN
      ERROR = - ABS(ERROR)
      ERRMSG = "Error in getting command argument No.2 as XMDF_RESAMPLE File"
      RETURN
    END IF
    ...
    ELSE
      ...
      ELSE
        IF( VERIFY( TRIM(CHAR_ARGUMENT) , "0123456789:" ) /= 0 ) THEN
          ERROR = - 1
          ERRMSG = "Illegal character in command argument No.3 as ITS_START:ITS_STRIDE:ITS_END"
          RETURN
        ELSE
          IF( SCAN( TRIM(CHAR_ARGUMENT) , ":" ) /= 0 ) THEN
            ...
            IF( SCAN( TRIM(CHAR_ARGUMENT) , ":" ) /= 0 ) THEN
              ...
              IF( SCAN( TRIM(CHAR_ARGUMENT) , ":" ) /= 0 ) THEN
                ERROR = - 1
                ERRMSG = "Too many colons in command argument No.3 as ITS_START:ITS_STRIDE:ITS_END"
                RETURN
              ELSE
