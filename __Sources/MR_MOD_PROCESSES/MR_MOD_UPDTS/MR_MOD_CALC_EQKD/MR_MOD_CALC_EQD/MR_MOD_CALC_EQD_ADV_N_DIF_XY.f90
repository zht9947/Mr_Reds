...
!***********************************************************************************************************************************
  SUBROUTINE MR_CALC_EQD_ADV_N_DIF_XY( NI , NJ , K , EQD_ADV_XY , EQD_DIF_XY )
    ...
    REAL   (FDRD_KIND) , INTENT(OUT) , DIMENSION(1:NI1(FDRD_KIND),1:NJ) :: EQD_ADV_XY , EQD_DIF_XY

    REAL   (FDRD_KIND) , DIMENSION(0:NI0(FDRD_KIND),1:NJ) :: EQD_QADV_XY_U , EQD_QDIF_XY_U
    REAL   (FDRD_KIND) , DIMENSION(1:NI1(FDRD_KIND),0:NJ) :: EQD_QADV_XY_V , EQD_QDIF_XY_V

    ALLOCATE( DIV(1:NI1(FDRD_KIND),0:NJ) )
      CALL MR_INTERP_XY_SS_V( NI , NJ , DI(:,:, K ) , DIV )
      CALL MR_CALC_QADV_N_QDIF_XY_SS_U( NI , NJ ,   &
      & DI(:,:, K ) , DIV , RB , U(:,:, K ) , EQD_QADV_XY_U , EKXY , VXYU(:,:, K ) , EQD_QDIF_XY_U )
    DEALLOCATE( DIV )

    ALLOCATE( DIU(0:NI0(FDRD_KIND),1:NJ) )
      CALL MR_INTERP_XY_SS_U( NI , NJ , DI(:,:, K ) , DIU )
      CALL MR_CALC_QADV_N_QDIF_XY_SS_V( NI , NJ ,   &
      & DI(:,:, K ) , DIU , RB , V(:,:, K ) , EQD_QADV_XY_V , EKXY , VXYV(:,:, K ) , EQD_QDIF_XY_V )
    DEALLOCATE( DIU )
