...
!***********************************************************************************************************************************
  SUBROUTINE MR_INIT_GRID_SYS_EXTEND( FILE_XMDF_NAME_EXTEND , NLOOPS , ERROR , ERRMSG )
    ...
    CALL MR_GET_NND_NEM( MESH_IN_XMDF_ID_EXTEND , NND2 , NEM2 , ERROR , ERRMSG )
    IF( ERROR < 0 ) THEN
      ERRMSG = TRIM(ERRMSG)//" /"//TRIM(XF_PATH_MESH_IN_XMDF)//" in file "//TRIM(FILE_XMDF_NAME_EXTEND)
      CALL MR_CLOSE_MESH_IN_XMDF( FILE_XMDF_ID_EXTEND , ERROR_DUMMY , ERRMSG_DUMMY )
      CALL MR_CLOSE_FILE_XMDF( FILE_XMDF_ID_EXTEND , ERROR_DUMMY , ERRMSG_DUMMY )
      RETURN
    ELSE IF( NND2 /= ((2*NI*NLOOPS+1)*(2*NJ+1)) ) THEN
      ERROR = - 5502
      ...
      CALL MR_CLOSE_FILE_XMDF( FILE_XMDF_ID_EXTEND , ERROR_DUMMY , ERRMSG_DUMMY )
      RETURN
    ELSE IF( NEM2 /= (NI*NLOOPS*NJ) ) THEN
      ERROR = - 5501
    ...
    CALL MR_READ_NDID( MESH_IN_XMDF_ID_EXTEND , NEM2 , NI*NLOOPS , NJ ,   &
    & EMIDW2 , NDIDW2 , NDIDU2 , NDIDV2 , NDIDO2 , ERROR , ERRMSG )
    IF( ERROR < 0 ) THEN
      ERRMSG = TRIM(ERRMSG)//" /"//TRIM(XF_PATH_MESH_IN_XMDF)//" in file "//TRIM(FILE_XMDF_NAME_EXTEND)
      CALL MR_CLOSE_MESH_IN_XMDF( FILE_XMDF_ID_EXTEND , ERROR_DUMMY , ERRMSG_DUMMY )
      CALL MR_CLOSE_FILE_XMDF( FILE_XMDF_ID_EXTEND , ERROR_DUMMY , ERRMSG_DUMMY )
