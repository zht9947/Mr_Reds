...
!***********************************************************************************************************************************
  SUBROUTINE MR_OUTPUT_AVERAGE( FILE_AVERAGE_NAME , T , ERROR , ERRMSG )
    ...
    WRITE( FILE_AVERAGE_ID , '( ("---------------") , 10("--------------------") ,   &
    & "T=", A ,/,"KSI",T15,"H (m)",T35,"f (Pa)",T75,"ma (Pa)",T115,"Tau (Pa)",T155,"q (m^2/s)",T195,"k (J/m^3)")' ) DUMMY_T_CHAR
    ...
    WRITE( FILE_AVERAGE_ID , '(" --------",T15,10(ES13.6,7X))' )   &
    & AVERAGE_PLAN_H , AVERAGE_PLAN_UNIT_FORCES(:) , AVERAGE_PLAN_UNIT_MOTION(:) ,   &
    & AVERAGE_PLAN_TBUV(:) , AVERAGE_PLAN_QUV(:) , AVERAGE_PLAN_UNIT_ENERGY
    DKSI = REAL(0.5,CARD_KIND) / REAL(NI,CARD_KIND)
    KSI  = DKSI / REAL(2.0,CARD_KIND)
    DO I = 1 , NI
      WRITE( FILE_AVERAGE_ID , '(X,F8.6,T15,10(ES13.6,7X))' )   &
      & KSI ,   &
      & AVERAGE_XSEC_H( I ) , AVERAGE_XSEC_UNIT_FORCES( I ,:) , AVERAGE_XSEC_UNIT_MOTION( I ,:) ,   &
      & AVERAGE_XSEC_TBUV( I ,:) , AVERAGE_XSEC_QUV( I ,:) , AVERAGE_XSEC_UNIT_ENERGY( I )
      KSI = KSI + DKSI
    END DO
