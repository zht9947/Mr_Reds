...
!***********************************************************************************************************************************
  SUBROUTINE MR_OUTPUT_AVERAGE( FILE_AVERAGE_NAME , T , ERROR , ERRMSG )
    ...
    WRITE( FILE_AVERAGE_ID , '( ("---------------") , 13("--------------------") ,   &
    & "T=", A ,/,"KSI",T15,"H (m)",T35,"f (Pa)",T75,"ma (Pa)",T115,"Tau (Pa)",T155,"Total Tau (Pa)",T175,"q (m^2/s)",T215,   &
    & "k (J/m^2)",T235,"k1 (J/m^2)",T255,"k2 (J/m^2)")' ) DUMMY_T_CHAR
    ...
    DUMMY_BASE = 0.0 ; DUMMY_REF = R0 * UVR * UVR * ZR
    ...
    AVERAGE_PLAN_UNIT_CIRC_INTENSITY = AVERAGE_PLAN_UNIT_CIRC_INTENSITY * ( DUMMY_REF - DUMMY_BASE ) + DUMMY_BASE
    AVERAGE_XSEC_UNIT_CIRC_INTENSITY = AVERAGE_XSEC_UNIT_CIRC_INTENSITY * ( DUMMY_REF - DUMMY_BASE ) + DUMMY_BASE
    ...
    WRITE( FILE_AVERAGE_ID , '(" --------",T15,13(ES13.6,7X))' )   &
    & AVERAGE_PLAN_H , AVERAGE_PLAN_UNIT_FORCES(:) , AVERAGE_PLAN_UNIT_MOTION(:) ,   &
    & AVERAGE_PLAN_TBUV(:) , AVERAGE_PLAN_TBUV_MOD, AVERAGE_PLAN_QUV(:) ,   &
    & AVERAGE_PLAN_UNIT_ENERGY , AVERAGE_PLAN_UNIT_CIRC_INTENSITY(:)
    DKSI = 0.5 / REAL(NI,CARD_KIND)
    KSI  = DKSI / REAL(2.0,CARD_KIND)
    DO I = 1 , NI
      WRITE( FILE_AVERAGE_ID , '(X,F8.6,T15,13(ES13.6,7X))' )   &
      & KSI ,   &
      & AVERAGE_XSEC_H( I ) , AVERAGE_XSEC_UNIT_FORCES( I ,:) , AVERAGE_XSEC_UNIT_MOTION( I ,:) ,   &
      & AVERAGE_XSEC_TBUV( I ,:) , AVERAGE_XSEC_TBUV_MOD( I ) , AVERAGE_XSEC_QUV( I ,:) ,   &
      & AVERAGE_XSEC_UNIT_ENERGY( I ) , AVERAGE_XSEC_UNIT_CIRC_INTENSITY( I ,:)
      KSI = KSI + DKSI
    END DO
